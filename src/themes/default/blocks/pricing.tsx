...
function getAvailablePaymentProviders(
	...
	const enabledProviders = ['stripe', 'creem', 'paypal'].filter((provider) => {
		return configs[`${provider}_enabled`] === 'true';
	});

	if (!item.payment_providers || item.payment_providers.length === 0) {
		return enabledProviders;
	}

	return enabledProviders.filter((provider) =>
		item.payment_providers?.includes(provider)
	);
}
...
export function Pricing({
	...
	const paymentT = useTranslations('common.payment');
	...
	const handlePayment = async (item: PricingItem) => {
		// if (pathname.endsWith('/pricing') && item.interval !== 'one-time') {
		//   toast.info(paymentT('subscription_coming_soon'));
		//   return;
		// }
		...
		if (availableProviders.length === 1) {
			handleCheckout(displayedItem, availableProviders[0]);
		} else if (
			...
			handleCheckout(
				displayedItem,
				availableProviders.includes(defaultProvider)
					? defaultProvider
	...
	return (
		...
		>
			...
			<div className="container">
				...
				<div
					className={`mx-auto mt-0 grid w-full gap-6 md:grid-cols-${section.items?.filter((item) => !item.group || item.group === group)
							?.length
						}`}
				>
